    ...
    const CancelOrder = async () => {
        try {
            const response = await fetch(
                `${import.meta.env.VITE_API_URL}/api/cancelorder/${props.id}`,
                {
                    method: "POST",
                    headers: {
        ...
        <>
            <div className="order-main">
                <div className="order" onClick={() => setHidden(!hidden)}>
                    <div className="desc">
                        <div className="name">
                            <p className="montserrat">{`Заказ от: ${props.name}`}</p>
                        </div>
                        <div className="price">
                            <p className="montserrat">
                                {`Стоимость: ${props.price}`} р
                            </p>
                        </div>
                    </div>
                    {status === "new" && (
                    ...
                    {props.status == "new" ? (
                        <div className="buttons">
                            <button
                                onClick={(e) => {
                                    (Button(e), CancelOrder());
                                }}
                            >
                                <p className="cancel">Отменить заказ</p>
                            </button>
                        </div>
                    ) : (
                        ""
