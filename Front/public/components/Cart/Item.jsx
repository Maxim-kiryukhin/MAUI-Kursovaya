...
function Item(props) {
    ...
    const MinusCart = async () => {
        try {
            const response = await fetch(
                `${import.meta.env.VITE_API_URL}/api/decreasegoodincart/${props.id}`,
                {
                    method: "POST",
                    headers: {
    ...
    const PlusCart = async () => {
        try {
            const response = await fetch(
                `${import.meta.env.VITE_API_URL}/api/increasegoodincart/${props.id}`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    return (
        <>
            <div className="cart-item">
                <div
                    className="pic"
                    onClick={() => props.GoodPage("Good", props.item.good)}
                >
                    <img
                        src={`${import.meta.env.VITE_API_URL}/${props.pic}`}
                        alt=""
                    />
                </div>
                <div className="desc">
                    ...
                    <div className="price">
                        <p className="montserrat">{`${props.price}`} р</p>
                    </div>
                </div>
                <div className="buttons">
                    <button
                        onClick={() => {
                            PlusCounter(SelectedAmount + 1);
                        }}
                    >
                        <p className="change">+</p>
                    </button>
                    <div className="amount">
                        <p className="montserrat">{SelectedAmount}</p>
                    </div>
                    <button
                        onClick={() => {
                            MinusCounter(SelectedAmount - 1);
                        }}
                    >
                        <p className="change">-</p>
                    </button>
                ...
                <div className="final-price">
                    <p className="montserrat">{`${ItemPrice} р`}</p>
                </div>
            </div>
        </>
    );
